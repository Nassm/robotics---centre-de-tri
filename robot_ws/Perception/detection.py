...
def turn_to_grey(imag):
    ...
    ##################
    # YOUR CODE HERE #
    return np.dot(imag[...,:3], [0.299, 0.587, 0.114])
    ##################
...
def perform_closing(imag):
    ...
    ##################
    # YOUR CODE HERE #
    return ndimage.grey_closing(imag, size=(3, 3))
    ##################
...
def perform_opening(imag):
    ...
    ##################
    # YOUR CODE HERE #
    return ndimage.grey_opening(imag, size=(3, 3))
    ##################
...
def clean_shape(imag):
    ...
    ##################
    # YOUR CODE HERE #
    return convex_hull_object(imag, 8)
    ##################
...
def extract_raw_contour(shape):
    ...
    We use the `find_contours` function from scikit-image, and checks whether
    a contour was found before returning the first one (which should be the
    only one). The array is flipped, since the dimensions are in the wrong
    order.
    """
    ##################
    # YOUR CODE HERE #
    contour = find_contours(shape, 0)
    if len(contour)==0:
        return None
    elif len(contour)==1:
        return np.flip(contour[0])
    else:
        raise Exception("The image contains multiple contours.")
    ##################
...
def simplify_contour(image, contour):
    ...
    # YOUR CODE HERE #    
    ##################
    def extract_corner(points, centroid):
        """
        Helper function allowing to extract the point most distant from the
        centroid.
        """
        distances = np.array([np.linalg.norm(centroid-p) for p in points])
        return points[distances.argmax()]

    # We extract the center of mass of the image , which is the center of
    # the shape, since on this binary image, the pixels on the cube are 1
    # and the background are only zeros.
    centroid = np.flip(ndimage.measurements.center_of_mass(image))
    # We separate the contour points in four blocks around the centroid
    left   = contour[:,0] <  centroid[0]
    right  = contour[:,0] >= centroid[0]
    top    = contour[:,1] <  centroid[1]
    bottom = contour[:,1] >= centroid[1]
    topleft     = contour[np.logical_and(top,    left)]
    bottomleft  = contour[np.logical_and(bottom, left)]
    topright    = contour[np.logical_and(top,    right)]
    bottomright = contour[np.logical_and(bottom, right)]
    # If the cut is broken, we raise an exception
    if not (topleft.tolist() and \
            bottomleft.tolist() and \
            topright.tolist() and \
            bottomright.tolist()):
        raise Exception("Failed to cut the contour")
    # We extract the point with largest distance from the centroid for each
    # block (presumably the corner)
    tl_corner = extract_corner(topleft,     centroid)
    bl_corner = extract_corner(bottomleft,  centroid)
    tr_corner = extract_corner(topright,    centroid)
    br_corner = extract_corner(bottomright, centroid)

    return np.array([br_corner, bl_corner, tl_corner, tr_corner])
...
def compute_transform(contour):
    ...
    tfr = tf.ProjectiveTransform()
    tfr.estimate(dst, src)
    return tfr
    ##################
...
def transform_image(imag, transform):
    ...
    ##################
    # YOUR CODE HERE #
    return tf.warp(imag, transform, output_shape=(28,28)) 
    ##################
